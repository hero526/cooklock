...
public class MyListAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
       ...
       // ImageView iconImageView = (ImageView) convertView.findViewById(R.id.recipe_image) ;
        TextView iconImageView = (TextView) convertView.findViewById(R.id.recipe_image) ;
        TextView titleTextView = (TextView) convertView.findViewById(R.id.recipe_name) ;

        // Data Set(listViewItemList)에서 position에 위치한 데이터 참조 획득
        Recipe_Basic recipe_basic = listViewItemList.get(position);

        // 아이템 내 각 위젯에 데이터 반영
        titleTextView.setText(recipe_basic.getRecipe_Image());
  //     iconImageView.setImageDrawable(recipe_basic.getRecipe_Image());
